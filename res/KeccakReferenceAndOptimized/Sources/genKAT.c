...
STATUS_CODES    genShortMsgEx(unsigned int rate, unsigned int capacity, unsigned char diversifier, int outputLength, const char *fileName);
#endif
...
#ifdef AllowExtendedFunctions
    if ( (ret_val = genShortMsg(0)) != KAT_SUCCESS )
...
#ifdef AllowExtendedFunctions
    /* Other cases */
    genShortMsgEx(1344, 256, 0, 4096, "ShortMsgKAT_r1344c256.txt");
#endif
...
#ifdef AllowExtendedFunctions
STATUS_CODES
genShortMsgEx(unsigned int rate, unsigned int capacity, unsigned char diversifier, int outputLength, const char *fileName)
{
    char        fn[32], line[SUBMITTER_INFO_LEN];
    int         msglen, msgbytelen, done;
    BitSequence Msg[256], MD[64];
